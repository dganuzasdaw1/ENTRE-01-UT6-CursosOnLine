...
public class Curso
{
    ...
    private String capitalizarNombre(String nombre) {
        String[] capitalizado = nombre.trim().split(ESPACIO);
        StringBuilder sb = new StringBuilder();
        for(int i = 0; i < capitalizado.length; i++){
            char primera = capitalizado[i].charAt(0);
            String mayuscula = Character.toString(primera).toUpperCase();
            sb.append(mayuscula + capitalizado[i].substring(1) + ESPACIO);
        }
        return sb.toString();
    }
